...
void CSV_parser::read(const std::string& filename)
{
	...
	while (std::getline(*pfile, line)) // Построчно читаем файл
	{
		...
		// Если читаем заголовок, то запоминаем размер для последующего сравнения
		// Добавляем для каждого названия свой столбец
		if (head)
		{
		...
		// Иначе проверяем, что текущая строка по размеру равна заголовку
		// Добавляем номер строки в вектор
		// Вставляем в нужную ячейку выражение
		else
		{
